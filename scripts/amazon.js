...
document.querySelectorAll('.add-to-cart-button')
  .forEach((button) => {
    button.addEventListener('click', () => {
      ...
      // below code is for the 'Added' message when we click 'Add to Cart'
      const addedElement = document.querySelector(`.js-added-to-cart-${productId}`);
      addedElement.classList.add("added-to-cart-visble");
      
      if(addedMessageTimeoutId[productId]){
        clearTimeout(addedMessageTimeoutId[productId]);
      }
      const timeoutId = setTimeout(()=>{
        addedElement.classList.remove("added-to-cart-visble");
      }, 2000);
